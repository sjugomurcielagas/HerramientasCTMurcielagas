...
- Mocks de backend vacío/caído: Reportes, Base, Concentraciones, Antidoping, Partidos y Penales no crashean ni generan overflow horizontal en 390x844; algunos módulos dejan errores de consola esperables por plantel/API y carga de `assets/config.js` relativa a subrutas.
...
| P2 | Cache/PWA | Transversal | La verificación de frescura busca `./assets/config.js` relativo a la ruta actual y genera 404 en subrutas. | Ruido de consola/red y menor confiabilidad del mecanismo de actualización. | Logs del servidor local: `/reportes/assets/config.js`, `/analisis/partidos/assets/config.js`, `/tactica/assets/config.js` devuelven 404. | `new URL('./assets/config.js', location.href)` no apunta a la raíz del sitio cuando la página está en una carpeta. | Calcular la URL desde el script cargado o desde una base raíz conocida; usar `../` correcto por módulo o `document.currentScript.src`. | S | Navegar cualquier subruta no genera requests 404 a `*/assets/config.js?t=...`; la verificación sigue funcionando. |
...
| P2 | UX/error | Transversal | Favicon 404 aparece como `console error` en mocks. | Ruido en monitoreo/Playwright si se endurece la política de consola. | Mock Playwright y logs locales reportaron `/favicon.ico` 404. | Falta ruta o asset esperado por browser. | Agregar favicon válido o ajustar manifest/head. | XS | Navegación con consola estricta no registra 404 de favicon. |
